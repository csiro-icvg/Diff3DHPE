# This file is modified from https://github.com/facebookresearch/VideoPose3D/blob/main/common/h36m_dataset.py
        ...
        for subject, actions in data.items():
            ...
            for action_name, positions in actions.items():
                ...
                for cam in self._cameras[subject]:
                    ...
                    all_pos_3d_in_camera_coo.append(pos_3d)

        all_pos_3d_in_camera_coo = np.concatenate(all_pos_3d_in_camera_coo, axis=0)
        all_pos_3d_in_camera_coo_centred = all_pos_3d_in_camera_coo - all_pos_3d_in_camera_coo[:, :1]
        self._pos_3d_min = all_pos_3d_in_camera_coo_centred.min()
        self._pos_3d_max = all_pos_3d_in_camera_coo_centred.max()
        self._w_mpjpe = torch.tensor([1, 1, 2.5, 2.5, 1, 2.5, 2.5, 1, 1, 1, 1.5, 1.5, 4, 4, 1.5, 4, 4])
        self._limb_start_list = []
        self._limb_end_list = []
        self._limb_name_list = []
        self._num_limb = len(limb_dict)
        for k, v in limb_dict.items():
            self._limb_name_list.append(k)
            self._limb_start_list.append(v[0])
            self._limb_end_list.append(v[1])
