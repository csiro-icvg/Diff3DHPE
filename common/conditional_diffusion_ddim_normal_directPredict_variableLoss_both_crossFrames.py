...
class GaussianDiffusion(nn.Module):
    ...
    def ddim_sample_loop(self, x_in, target_shape):
        ...
        for time, time_next in time_pairs:
            ...
            x_start = self.ddim_sample(noisy_3D_pose, time, condition_x=x_condition)
    ...
    def p_losses(self, x_start, pose_2d, noise=None):
        ...
        if not self.conditional:
            model_out = self.model.forward_denoise(x_noisy, t)
        else:
            x_condition = pose_2d

            model_out = self.model.forward_denoise(
                torch.cat([x_condition, x_noisy], dim=-1), t)
